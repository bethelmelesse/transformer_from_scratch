...
class Attention(nn.Module):
    ...
    def forward(self, input_embeddings, token_attention_masks):                        # x = 2, 16, 128
        ...
        new_query = torch.reshape(query, (batch_size, seq_length, head_dim, num_heads))      # shape = 2, 16, 16, 8
        ...
        new_query = torch.reshape(new_query, (batch_size * num_heads, seq_length, head_dim))         # shape = 16, 16, 16
         ...
         # task 3.2: for key
        new_key = torch.reshape(key, (batch_size, seq_length, head_dim, num_heads))             # shape = 2, 16, 16, 8
...
class Model(nn.Module):
    def __init__(self, num_layers):
        ...
        for _ in range(num_layers):
            self.layers.append(Transformer_layer(num_layers))
