...
def main():
    ...
    def source_dataset(source_context_path):
        # Source Sequence
        with open(source_context_path, encoding='utf8') as f:
            source_contexts = [source_context.strip() for  source_context in f.readlines()][:2]
        # Tokenizer - source
    ...
    def target_dataset(target_context_path):
        # Target Sequence
        with open(target_context_path, encoding='utf8') as f:           
            target_contexts = [target_context.strip() for  target_context in f.readlines()][:2]
        # Tokenizer - target
    ...
    vocab_size_source, token_input_ids_source, token_attention_masks_source = source_dataset(source_context_path)
    vocab_size_target, token_input_ids_target, token_attention_masks_target, vocab_target = target_dataset(target_context_path)
